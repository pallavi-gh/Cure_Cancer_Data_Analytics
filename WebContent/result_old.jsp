...
%>
    ...
    <div id="body">
		...
		if(Integer.toString(i).length() == 1){
			img_name1 = "path-image-10";
		}
		else{
			img_name1 = "path-image-1";
		}
		
		if(Integer.toString(i+1).length() == 1){
			img_name2 = "path-image-10";
		}
		else{
			img_name2 = "path-image-1";
		}
		
		if(Integer.toString(i+2).length() == 1){
			img_name3 = "path-image-10";
		}
		else{
			img_name3 = "path-image-1";
		}
		%>
		...
		<td><img class="image_scale" src='<%="images/output/"+image_icon[(int)idx.get(i)] %>' alt="Input_image" style="width:100px;height:100px">
				...
				<div><a href='<%="images/highlight/"+img_name1+i+"-0.jpg" %>'><%=image_name[(int)idx.get(i)]%></a></div></td>
		<td><%
		if((i+1) < idx.size()){ 
	%>  <img class="image_scale" src='<%="images/output/"+image_icon[(int)idx.get(i+1)] %>' alt="Input_image" style="width:100px;height:100px">
				...
				<div><a href='<%="images/highlight/"+img_name2+(i+1)+"-0.jpg" %>'><%=image_name[(int)idx.get(i+1)]%></a></div><%}
		else{
			continue;
		}%></td>
		<td><%
		...
		<img class="image_scale" src='<%="images/output/"+image_icon[(int)idx.get(i+2)] %>' alt="Input_image" style="width:100px;height:100px">
				...
				<div><a href='<%="images/highlight/"+img_name3+(i+2)+"-0.jpg" %>'><%=image_name[(int)idx.get(i+2)]%></a></div>
		<%}
		else{
			continue;
		}%></td>
